...
if($controller == 'HomeController'){
    include_once "{$_SERVER['DOCUMENT_ROOT']}/BTTH3/app/controllers/HomeController.php";
    
    $homeController = new $controller();
    $homeController->$action();
}
else if ($controller == "AdminController"){
    include_once "{$_SERVER['DOCUMENT_ROOT']}/BTTH3/app/controllers/AdminController.php";
    $adminController = new $controller();
    $adminController->$action();
    if ($action == 'indexAuthor') {
        $adminController->indexAuthor();
    } else {
        $adminController->$action();
    }
}
